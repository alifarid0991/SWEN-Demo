...
class VideoIndexer():
    ...
    def upload_to_video_indexer_by_url(self, input_url, video_name, video_language='en-US'):
    ...
    def search_in_video(self, query, language='en-US'):
        self.check_access_token()

        print('Search in video indexer...')
        params = {
            'query': query,
            'language': language,
            'pageSize' : "25" ,
        ...
        upload_video_req = requests.get(
            'https://api.videoindexer.ai/{loc}/Accounts/{acc_id}/Videos/Search'.format(
                loc=self.vi_location,
                acc_id=self.vi_account_id
            ),
            params=params,
